...
export default function Kitchen() {
  ...
  const [ws, setWs] = useState<KitchenWebSocket | null>(null);
  
  // Initialize WebSocket connection
  useEffect(() => {
    const kitchenWs = new KitchenWebSocket(floor);
    
    const removeConnectHandler = kitchenWs.onConnect(() => {
      setConnected(true);
      toast({
        title: "Connected",
        description: `Connected to kitchen for Floor ${floor}`,
      });
    });
    
    const removeErrorHandler = kitchenWs.onError(() => {
      setConnected(false);
      toast({
        title: "Connection Error",
        description: "Failed to connect to kitchen. Retrying...",
        variant: "destructive",
      });
    });
    
    const removeMessageHandler = kitchenWs.onMessage((data: WebSocketMessage) => {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      {/* Floor Selector */}
      <FloorSelector currentFloor={floor} urlPrefix="/kitchen" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {orders.length > 0 ? (
          ...
          <div className="col-span-full text-center py-8 bg-white rounded-lg shadow-md">
            <p className="text-gray-500">No active orders for this floor.</p>
          </div>
        )}
      </div>
    </div>
