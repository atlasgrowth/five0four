...
export async function syncCatalog() {
  const resp = await catalogApi.listCatalog({ types:["ITEM","ITEM_VARIATION","CATEGORY"] });
  const objs = resp.result.objects ?? [];

  const categories = Object.fromEntries(
    objs.filter(o=>o.type==="CATEGORY").map(c=>[c.id, c.categoryData?.name])
  );

  const rows = objs.filter(o=>o.type==="ITEM_VARIATION").map(v=>{
...
if (import.meta.url.endsWith("syncSquareCatalog.ts")) syncCatalog();
